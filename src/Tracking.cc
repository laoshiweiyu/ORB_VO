...
Tracking::Tracking(System *pSys, Map *pMap, const string &strSettingPath, const int sensor):
    mState(NO_IMAGES_YET), mSensor(sensor), mpSystem(pSys), mpMap(pMap), mnLastRelocFrameId(0)
{
...
void Tracking::Track()
{
	...
	if(mState==OK)
	{
	    ...
	    if(mVelocity.empty() || mCurrentFrame.mnId<mnLastRelocFrameId+2)
	      {
		std::cout << "tracking via reference key frame\t";
		bOK = TrackReferenceKeyFrame();
		std::cout << (bOK?"OK":"NOT OK")<< std::endl;				
	      }
	    else
	      {
		std::cout << "tracking via motion model\t";
	...
	{
	    std::cout << "mState != OK.. Need relocalization" << std::endl;
	    bOK = false;
	}
